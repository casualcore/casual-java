...
class CompleteCasualServiceCallRequestMessageTest extends Specification
{
   ...
   def 'roundtrip message protocol version 1.4'()
   {
    ...
    def "roundtrip message protocol version equal or greater to one three"()
    {
       ...
       when:
       CasualServiceCallRequestMessage msg = CasualNetworkTestReader.readMessage(CasualNWMessageType.SERVICE_CALL_REQUEST, sink, 205, ProtocolVersion.VERSION_1_3)
       write(sink, msg)
       CasualServiceCallRequestMessage resurrectedMsg = CasualNetworkTestReader.readMessage(CasualNWMessageType.SERVICE_CALL_REQUEST, sink, 205, ProtocolVersion.VERSION_1_3)
       then:
       msg != null
       msg == resurrectedMsg

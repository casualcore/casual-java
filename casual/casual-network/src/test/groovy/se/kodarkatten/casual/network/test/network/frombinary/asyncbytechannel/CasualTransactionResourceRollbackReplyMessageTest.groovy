...
{
    @Shared
    def resource = '/protocol/bin/message.interdomain.transaction.resource.receive.rollback.Reply.bin'
    ...
    def "roundtrip message - no chunking"()
    {
        ...
        when:
        CasualNWMessage<CasualTransactionResourceRollbackReplyMessage> msg = CasualNetworkReader.read(sink)
        CasualNetworkWriter.write(sink, msg)
        CasualNWMessage<CasualTransactionResourceRollbackReplyMessage> resurrectedMsg = CasualNetworkReader.read(sink)
        then:
        msg != null

...
When a connection is accepted, `casual-jca` issues a domain connect request as it does for a normal outbound connection. The reverse inbound side waits for the outbound side to initiate the handshake.
...
If no EIS connection is available during a connection request, allocation fails in the same way as a standard outbound casual outage. You can retry acquisition after the EIS reconnects.
